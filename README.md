...
検証要件に合わせ、NEWおよびPENDING画面では「対応中」と「完了」を表示します。「完了」はサーバー側で不正遷移として拒否され、エラーメッセージが表示されます。状態遷移ルール自体は変更していません。

担当者候補と操作可否はサーバーが返す権限情報に従って表示します。MEMBERは未割り当て時の自分だけ、ADMINは有効ユーザー全員を選択できます。「未割り当て」はADMINかつNEWの場合だけ選択できます。
